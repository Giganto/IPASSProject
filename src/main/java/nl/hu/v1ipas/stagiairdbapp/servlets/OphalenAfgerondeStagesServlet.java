...
@WebServlet(urlPatterns = "/OphalenAfgerondeStages.do")
public class OphalenAfgerondeStagesServlet extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		out.println(" <form action=\"VergoedingBerekenen.do\"><table id=\"myTable\"><tr><th>Voornaam</th><th>Achternaam</th><th>Afdeling</th><th>Niveau</th><th>Type</th>");
		for(int x = 0; x < i; x+=5){
			out.println("<tr><td>" + stages.get(x) + "</td><td>" + stages.get(x+1) + "</td><td>" + stages.get(x+2) + 
					"</td><td>" + stages.get(x+3) + "</td><td>" + stages.get(x+4) + "</td></tr>");
		}
		out.println("</table>");
		out.println("<input type=\"submit\" value=\"Vergoeding Berekenen\" /><br>");
		out.println("</form>");
